...
update_squid_bad_urls() {
    echo "$(date +\%Y-\%m-\%d\ %H:\%M:\%S) Updating squid bad URLs..." >> /path/to/your/logfile.log
    local output_file="urlhaus_data.csv"
    ...
    curl -o "$output_file" "https://urlhaus.abuse.ch/downloads/csv_online/"
    
    # Calculate the hash of the newly downloaded CSV data
    local new_hash=$(sha256sum "$output_file" | cut -d ' ' -f 1)
...
update_IpFeed_Feodotracker() {
    echo "$(date +\%Y-\%m-\%d\ %H:\%M:\%S) Updating IpFeed Feodotracker..." >> /path/to/your/logfile.log	
    local output_file="bad_ips_data.txt"
    ...
    curl -o "$output_file" "https://feodotracker.abuse.ch/downloads/ipblocklist.txt"
